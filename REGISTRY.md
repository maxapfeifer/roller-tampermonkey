...
| A31 | **Mandatory "Reason for this change" on member name edits** — after "I understand — edit", a required reason box appears first and the Name field stays locked until a reason is typed; the reason is put in the name-change audit email and stored nowhere else (memory only). Audit emails also send the **full clickable URL** (not just the path tail) | An audit trail of member renames needs the WHY, and staff must be forced to justify each change; admin wants clickable links in the email | `REQUIRE_NAME_REASON`, `NAME_REASON_LABEL`, `NAME_REASON_PLACEHOLDER` | `name-lock` |
| A32 | **Photo-override audit email** — when an EXISTING member photo is replaced (not a first-time add), email the admin with the **old + new photo attached**. Detects override via the capture edit-button; grabs the new frame from `<video>` at Capture (live-camera + file-upload paths); sends on Done/Save | An overwrite of a member photo should leave a trail (who/when + both images), while adding a first photo is routine and not reported | `AUDIT_PHOTO_OVERRIDE`, `PHOTO_MAX_PX` | (audit endpoint; needs Apps Script `UrlFetchApp` auth for the old-photo attach) |
