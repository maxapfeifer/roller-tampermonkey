...
| A18b | On a **party booking**, admission tiles' big label reads **"Party Guest"** instead of Adult/Child | Flag party attendees at a glance; a booking is a party when it carries a form whose name mentions "party" (detected via `<app-booking-forms>`, scoped so the Party Room resource / party discount don't false-trigger). Non-admission items (food/add-ons) keep their real type | `FLAG_PARTY_GUESTS`, `PARTY_GUEST_LABEL` | — |
